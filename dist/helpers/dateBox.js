...
const formatEventDateByTimeZone = _ref => {
  ...
  if (currentTimezone.includes("GMT")) {
    currentTimezone = _constants.TIMEZONE_LIST.find(item => item.offset === currentTimezone).tzName;
  }
  ...
  if (!convertDate) {
    _start = (0, _momentTimezone.default)(_start).tz(currentTimezone).format(format);
    _end = (0, _momentTimezone.default)(_end).tz(currentTimezone).format(format);
  }
  return {
    start: _start,
    end: _end

...
function downloadSharer(e, type, event) {
  ...
  let icsSharer = "https://calendar.boomte.ch/createIcsFile?title=".concat(event.title, "&desc=").concat(encodeURIComponent(type === 'icalendar' ? desc.replace(/(<([^>]+)>)/ig, '') : desc), "&start=").concat(event.start.replace(/[-,:]/g, '') + '00', "&end=").concat(event.end.replace(/[-,:]/g, '') + '00', "&address=").concat(encodeURIComponent(event.venue.address));

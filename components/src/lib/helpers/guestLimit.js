...
export const getRegistrationProperties = ({
    ...
    if (eventKind === 4) {
        ...
        const { page_url, site_type, limit, limit_type, hide_reg_button } =
            general;
...
export const getGuestLimitProperties = (props) => {
    ...
    if (eventKind === 4) {
        ...
        const { registration_enabled, page_url, rsvp, site_type } =
            registration;
...
export const getGuestsCount = (addons, eventTicket, guests = [], startDate) => {
    ...
    const compareDates = ({ guestDate, guestId }) => {
        ...
        if (guestId < 80420) {
            ...
            return (
                momenttimezone(guestDate).format(
                    formats[+guestDate.includes("T")]
                ) ===
                momenttimezone(startDate).format(
                    formats[+startDate.includes("T")]
                )
            );
        }
    };

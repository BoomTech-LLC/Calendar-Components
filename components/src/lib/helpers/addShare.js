...
export function downloadSharer(e, type, event, instance) {
  ...
  let desc = `${event.desc
      ...
      : ""
    }${event.venue.name ? `${event.venue.name},<br/>  ` : ""}${event.venue.phone ? `${event.venue.phone},<br/>  ` : ""
    }${event.venue.email ? `${event.venue.email},<br/>  ` : ""}${event.venue.website ? `${event.venue.website}.<br/>  ` : ""
      ...
      ? "<p><b>Organizer</b></p>  "
      : ""
    }${event.organizer.name ? `${event.organizer.name},<br/>  ` : ""}${event.organizer.phone ? `${event.organizer.phone},<br/>  ` : ""
    }${event.organizer.email ? `${event.organizer.email},<br/>  ` : ""}${event.organizer.website ? `${event.organizer.website}.<br/>  ` : ""
    }`;
  ...
  switch (type) {
    case "google":
      ...
      if (event.all_day)
        ...
        ).format("YYYYMMDD")}&details=${event.desc ? eventDescription : ""
        ...
        &dates=${momenttimezone(
          ...
          formatForAddtoCalendar(event, "end")
        ).format("YYYYMMDD[T]HHmmss")}&details=${event.desc ? eventDescription : ""
          }&location=${setLocation(event, "encode")}`;
      break;
    case "yahoo":
...
export function generateEventUrl(
  ...
  const url = `${boomEventUrlBase}${encodeId(`${event.id}`)}${addDateInUrl ? `?date=${dateParams.join(",")}` : ""
    }`;
